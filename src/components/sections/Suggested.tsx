...
const Suggested: React.FC<SuggestedProps> = ({ id }) => {
  const [data, setData] = useState<Movie[]>([]);
  useEffect(() => {
    const fetchSuggested = async () => {
      try {
        const response = await getRecommendation(Number(id));
        setData(response);
      } catch (error) {
        console.error(error);
      }
  ...
  return (
    <section className="w-full bg-darkPrimary px-8 py-8 text-white">
      <h2 className="font-nunito text-xl font-medium">Suggested Movies</h2>
